...
// ######################################################################
public class K_OrbitCamera : MonoBehaviour
{
    ...
    public float default_mDistane = 3f;
    ...
    void OnEnable()
    {
        ...
        m_Distance = default_mDistane; // default value
                                       //  Debug.Log("M distance :" + m_Distance);
        ...
        if (m_Target)
        {
            ...
            if (Input.touchCount == 2)
            {
                ...
                if (m_ZoomMethod == ZoomMethod.PinchZoom)
                {
                    if (Input.touchCount == 2)
                    {
                        ...
                        m_Distance += delta * m_ZoomSpeed * 0.01f * Time.deltaTime;
                        if (m_Distance < m_MinDistance)
                        {
                            m_Distance = m_MinDistance;
                        }
            ...
            if (ZoomedbyTouch == false)
            {
                if (Input.GetAxis("Mouse ScrollWheel") < -deadZone || Input.GetAxis("Mouse ScrollWheel") > deadZone)
                {
                    ...
                    m_Distance -= delta * m_ZoomSpeed * Time.deltaTime;
                    //Debug.Log("zoom mouse: "+m_Distance);
                    if (m_Distance < m_MinDistance)
                    {
                        m_Distance = m_MinDistance;
                    }
            ...
            if (!ZoomedbyTouch)
            {
                // Debug.Log("zoom by touch :" + ZoomedbyTouch);
                ...
                if (Input.touchCount == 1)
                {
                    ...
                    else if (touch.phase == TouchPhase.Moved)
                    {
                        ...
                        if (m_YInvert)
                            TouchDeltaY *= -1;
                        // Debug.Log("Zoom in");
                        m_X += touch.deltaPosition.x * m_XSpeed * 0.005f * m_OrbitSpeedDelayTime * Time.deltaTime;
                        m_Y -= touch.deltaPosition.y * m_YSpeed * 0.005f * m_OrbitSpeedDelayTime * Time.deltaTime;
                    }
                }
            }
            // Orbit by mouse
            if (OrbitByTouch == false)
            {
                ...
                if (Input.GetMouseButton(0))
                {
                    ...
                    m_X += MouseAxisX * m_XSpeed * 0.02f * m_OrbitSpeedDelayTime * Time.deltaTime;
                    m_Y -= MouseAxisY * m_YSpeed * 0.02f * m_OrbitSpeedDelayTime * Time.deltaTime;
    ...
    void UpdatePosition()
    {
        ...
        //Quaternion rotation = Quaternion.Euler(0, m_X, 0);
        // Debug.Log("Mini distance :" + m_Distance);
        Vector3 direction = new Vector3(0.0f, 0.0f, -m_Distance);
        Vector3 position = (Quaternion.Euler(m_Y, m_X, 0) * direction) + m_Target.position;
